...
import Ale.Core.Message (JobOffer, Message)
import Ale.Core.Storage.Types (HasPrefixTag (..), PrefixTag, StoragePrefix (..))
import Ale.State (AleStateData)
...
-- | A marker for datatypes that can be persisted to a database.
class HasPrefixTag a => Persistable a where

instance Persistable ()
instance Persistable AleStateData
instance Persistable BlockPointer
instance Persistable Height
instance Persistable Block
instance Persistable PublicKey
instance Persistable JobOffer
instance Persistable Message
instance Persistable a => Persistable [a]
instance Persistable a => Persistable (MessageSet a)
instance Persistable a => Persistable (Signed a)
...
-- | Capability the allow one to store data in a database.
-- 'iter' is a database-specific iterator type.
data DB m = DB
...
putDB' k val = putDBRaw (getPrefix @v) k (strictSer val)

delDB' :: forall v m . (MonadDB m, Persistable v) => ByteString -> m ()
delDB' = delDBRaw (getPrefix @v)
...
getPrefix :: forall value . HasPrefixTag value => PrefixTag
getPrefix = getStoragePrefix $ storagePrefix @value
...
-- | Fetch element by key from collection.
getDBCollection :: forall m e k ks i. (Monad m, MonadDB m, Collectible k ks, Persistable i)
                => Collection e (k:ks) i
                -> ColKeyRep (k:ks)
                -> m (Either Text e)
getDBCollection c k = bimap show (cRecollect c k) <$> getDB' (collectionKeyByKey c k)
...
delDBCollection :: forall m e k ks i. (Collectible k ks, MonadDB m, Persistable i)
                ...
                -> e
                -> m ()
delDBCollection c e = delDB' @i (collectionKey c e)
...
collectionKeyByKey :: forall e k ks i. Collectible k ks
                   => Collection e (k:ks) i
                   -> ColKeyRep (k:ks)
                   -> ByteString
collectionKeyByKey c k = cName c <> "/" <> collectionKey' @k @ks k
