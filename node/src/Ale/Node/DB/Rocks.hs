...
import Ale.Node.DB.Types (DB (..), DBError (..), EntryOperations (..), Persistable, rawKey)
...
entryName pref k = getStoragePrefix pref <> "/" <> k
...
withRocksDBIterator db iprefix idropLength cont = cont ops (bracketP openIter closeIter produce)
  where
    ...
    prefix = entryName (storagePrefix @value) iprefix
...
withDBImpl :: (MonadIO m, MonadMask m, MonadBaseControl IO m,
               HasNoCap DB caps)
           => FilePath
