...
module Test.Ale.Node.DB.Rocks where
...
import Ale.Node.DB.Types



instance HasPrefixTag Word32 where
    storagePrefix = StoragePrefix "w32"
...
runWithTestDB :: (MonadIO m, MonadMask m, MonadBaseControl IO m) => CapsT '[DB] m a -> m a
runWithTestDB x =
    withSystemTempDirectory "ale-db-test" $ \dbPath ->
